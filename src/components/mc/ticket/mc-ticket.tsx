...
export default function MCTicket({ position, userId, ticketUsed }: MCTicketProps) {
  const [isTicketModalOpen, setIsTicketModalOpen] = useState(false);
  const [isTicketUsed, setIsTicketUsed] = useState(ticketUsed);

  const handleModalToggle = () => setIsTicketModalOpen(prev => !prev);
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black/95 z-mc-ticket-modal">
      <div className="relative w-svw h-svh flex flex-col justify-center items-center gap-7">
        ...
        <Button
          ...
          className="absolute bottom-4 left-4 sm:bottom-6 sm:left-6 md:bottom-8 md:left-8 lg:bottom-10 lg:left-10 xl:bottom-12 xl:left-12 rounded-full w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 lg:w-20 lg:h-20 xl:w-24 xl:h-24 2xl:w-28 2xl:h-28"
        >
    ...
    <>
      {!position ? (
        ...
        <div
          ...
          {...attributes}
          {...listeners}
          className={`object-contain fixed z-mc-ticket left-4 bottom-16 flex justify-center items-center ${isTicketUsed ? 'cursor-not-allowed brightness-50' : 'cursor-pointer'}`}
